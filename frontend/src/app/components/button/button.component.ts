...
export class ButtonComponent {
  ...
  get buttonClasses(): string {
    ...
    const s = { sm: 'px-4 py-2 text-sm', md: 'px-6 py-3 text-base', lg: 'px-8 py-4 text-lg' }[this.size];
    return `inline-flex items-center justify-center font-medium rounded-full transition-all duration-300 outline-none focus:ring-2 focus:ring-zinc-400 focus:ring-offset-2 dark:focus:ring-offset-zinc-950 disabled:opacity-40 disabled:cursor-not-allowed ${v} ${s} ${this.fullWidth ? 'w-full' : ''} ${this.className}`;
  }
}
